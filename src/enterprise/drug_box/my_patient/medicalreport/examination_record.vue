        ...
        <el-table :data="tableData" border tooltip-effect="dark" style="width: 100%;"  slot="tablebox">
          ...
          <el-table-column fixed="right" label="操作" width="300">
            <template slot-scope="scope">
              <el-button @click="handleEdit(scope.$index, scope.row)">查看体检报告</el-button>
              <el-button @click="uphandleEdit(scope.$index, scope.row)">上传体检报告</el-button>
  ...
  export default {
      ...
      methods: {
        handleEdit(index, row) {
          this.$emit('gethandle-edit',{conname:'PhoTos',python:0,patientids:row.province,patientname:row.name,stages:'case', index:index, row:row});
        },
        uphandleEdit(index, row) {
          this.$emit('gethandle-edit',{conname:'upPhoto',python:0,patientids:row.province,patientname:row.name,stages:'case', index:index,row:row});
        },
